...
with its own design once this lands.
...
- Filesystem overrides: keep TOML-only, or accept `.json` too? (Proposal:
  accept both; JSON is what the export produces, so power users can copy one
  out and edit it.)
- Should `mise install` grow a pinned `cue` version now, or only in PR 1?
- `providers/cue/` at repo root vs `internal/system/definitions/cue/` —
  proposal is repo root: it is source, not an embedded asset.

...
# Use the session RWR unlocked and explicitly exposed for this blueprint.
...
if ! command -v bw >/dev/null 2>&1; then
  echo "gpg-restore: Bitwarden is not installed - skipping"
  exit 0
fi
...
case "$session_status" in
  unlocked) ;;
  locked) echo "gpg-restore: vault locked - skipping; unlock Bitwarden when ready"; exit 0 ;;
  unauthenticated) echo "gpg-restore: not logged in to Bitwarden - skipping"; exit 0 ;;
